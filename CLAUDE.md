...
## 아키텍처
- 옵션 C 하이브리드: v1 DataSet 도메인(backend-core) + v2 Port-Adapter(backend-bff) + vue-spring-fw UI 컴포넌트 복사(ui)
...
## vue-spring-fw 취급 규칙 (최상위)
- `C:\claude\vue-spring-fw\**` **원본 절대 수정 금지**
- 컴포넌트/composable/store/interceptor/템플릿은 읽기 후 `ui/` 로 **정적 복사**
- 복사 이력은 `docs/vue-spring-fw-reuse-map.md` 에 (원본경로 → v3경로 → 수정사항) 기록
