...
- **backend-bff** — WebFlux 기반 Port-Adapter, 외부 시스템(Keycloak/Mattermost/Stalwart/MinIO/LiveKit/Wiki.js) 추상화
- **ui** — Vue 3 + PrimeVue 4 + Pinia + keycloak-js (ts-spring-fw 컴포넌트 정적 복사)
- **인프라 9종** — PostgreSQL / Redis / Keycloak / MinIO / Mattermost / Stalwart / LiveKit / Wiki.js / Traefik
...
```
 ...
 └──────┬─────┘        └──────┬───────┘ └───┬──┘ └───┬────────┬──┘
        ...
        │        ▼           ▼               ▼       ▼        ▼
        │  ┌──────────┐ ┌───────────┐  ┌──────────┐ ┌────────┐ ┌──────────┐
        │  │  MinIO   │ │Mattermost │  │ Stalwart │ │LiveKit │ │ Wiki.js  │
...
# 3) 외부 시스템 어댑터 대상
docker compose -f infra/docker-compose.yml up -d mattermost stalwart livekit wikijs
...
|---|---|---|---|---|
| 1 | PostgreSQL 16 | v3-postgres | 19432:5432 | 전체 RDBMS (platform_v3 / flowable_v3 / mattermost_v3 / wiki_v3 / keycloak) |
...
| 5 | Mattermost | v3-mattermost | 19065:8065 | 메신저 (GitLab OAuth → Keycloak) |
...
└───┬────┘                      └────┬─────┘                      └─────┬─────┘
    ...
    │                                │                                  │
    │   동일 토큰으로 확장 SSO:                                          │
    │   Mattermost (GitLab OAuth client id=mattermost-v3)                │
...
| Mattermost | http://localhost:19065 | SSO only |
...
**4) Mattermost SSO "Invalid state parameter"**
...
- Rocket.Chat / Mattermost Mongo / Postgres dump
